...
import { IRoleInfo } from "../userList";
...
interface IAppsFilterProps extends StyledComponentProps {
  ...
  onSubmit: (value: IAddUserValue) => void;
  roleList?:IRoleInfo[];
  initialValues?: Partial<IAddUserValue>;
}
    ...
    return (
      <form onSubmit={handleSubmit}>
        <Box m={-2} mt={2}  className={classes.root}>
          ...
          <Box p={2}>
            <Box pb={1}>{t("user-info.dialog.addUser.username")}</Box>
            <Field name="username" component={InputField} margin="none" />
          </Box>
          <Box p={2}>
            <Box pb={1}>{t("user-info.dialog.addUser.password")}</Box>
            <Field name="password" component={PasswordInputField} margin="none" />
          </Box>
          <Box p={2}>
...
const AddUserDialog = withStyles(styles)(
